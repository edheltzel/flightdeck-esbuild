...
<details>
  <summary>Available Run Commands</summary>
		Lifecycle scripts:
			start
				eleventy --serve & pnpm run tw:dev

		Commands available via "pnpm run":
			dev
				pnpm run clean && pnpm run start
			build
				pnpm run clean && ENV=prod eleventy & pnpm run tw:build
			tw:build
				tailwindcss -i ./src/assets/styles/app.css -o ./dist/assets/styles/app.css --minify
			tw:dev
				tailwindcss -i ./src/assets/styles/app.css -o ./dist/assets/styles/app.css --watch
			debug
...
- `upgrade` command - uses `pnpx npm-check-updates -ui` to upgrade dependencies to their latest versions and updates the `package.json` - this is a work around for `pnpm run upgrade` not working as expected or how other package managers work.
